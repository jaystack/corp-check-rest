...
export class Validation extends CorpjsCorpcheckService {
    public async handle(
        ...
        let item = await getPackageResult({ name: packageInfo.name, version: packageInfo.version })
        ...
        if (item === null) {
            ...
            await startPackageValidation({ packageJSON: item.packageJSON, cid: item.id })
        }
...
export class Package extends CorpjsCorpcheckService {
    public async handle(
        ...
        const item = await getPackageResult({ name, version, cid })
...
@use(ErrorTransform)
@rest({ path: '/complete', methods: ['post'] }) //TODO remove
export class Complete extends CorpjsCorpcheckService {
    public async handle(
        @param cid,
        @param data,
